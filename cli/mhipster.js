...
if (!process.argv.includes('micronaut') && process.argv.includes('--blueprints')) {
    for (let i = 0; i < process.argv.length; i++) {
        if (process.argv[i] === '--blueprints') {
            process.argv[i + 1] = `${process.argv[i + 1].split(',')},micronaut`;
        }
    }
    // User passes in blueprint flag but without micronaut :> append micronaut
} else if (!process.argv.includes('micronaut') && process.argv.includes('--blueprint')) {
    for (let i = 0; i < process.argv.length; i++) {
        if (process.argv[i] === '--blueprint') {
            process.argv[i] = '--blueprints';
            process.argv[i + 1] = `${process.argv[i + 1]},micronaut`;
        }
    }
    // User donot pass in blueprints or blueprint flag but without micronaut :> append micronaut
} else if (!process.argv.includes('micronaut') && !process.argv.includes('--blueprint') && !process.argv.includes('--blueprints')) {
    process.argv.push('--blueprints');
    process.argv.push('micronaut');
}
